...
var FFP = (function() {
  ...
  function showDemoBanner() {
    ...
    b.innerHTML = '<div style="display:flex;align-items:center;gap:10px">'
      ...
      + 'text-decoration:none;border:1px solid rgba(21,101,192,.3);'
      + 'border-radius:5px;padding:5px 11px">← Autre profil démo</a>'
      + '<a href="ffp-login.html" onclick="FFP.exitDemo();return false;" style="font-size:10px;font-weight:700;color:#4BC88A;'
      + 'text-decoration:none;border:1px solid rgba(75,200,138,.35);'
      + 'border-radius:5px;padding:5px 11px">🔐 Connexion réelle</a>'
      + '<a href="ffp-pricing.html" style="font-size:10px;font-weight:800;'
      + 'background:linear-gradient(135deg,#0D1B3E,#1565C0);color:#fff;'
      + 'border-radius:5px;padding:5px 12px;text-decoration:none">Créer un compte →</a>'
      + '<button onclick="FFP.exitDemo()" title="Quitter le mode démo" style="background:transparent;'
      + 'border:1px solid rgba(255,255,255,.15);border-radius:5px;'
      + 'color:#888;padding:5px 10px;cursor:pointer;font-size:11px;font-weight:600">Déconnexion ✕</button>'
      + '</div>';
